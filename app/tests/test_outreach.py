...
class TestOutreachSystem(unittest.TestCase):
    ...
    def test_autonomous_email_generation_no_campaign(self):
        ...
        self.assertIn("subject", result)
        self.assertIn("body", result)
        self.assertIn("Google Maps SEO", result["body"])
        self.assertIn("WhatsApp Integrations", result["body"])
    ...
    def test_smtp_sender_test_mode(self, mock_smtp_class):
        ...
        # Force test mode active and valid SMTP host settings, bypassing Gmail token file if present
        with patch.object(settings, "EMAIL_TEST_MODE", True), \
             patch.object(settings, "EMAIL_TEST_RECIPIENT", "admin@example.com"), \
             patch.object(settings, "SMTP_HOST", "smtp.sendgrid.net"), \
             patch.object(settings, "SMTP_PORT", 587), \
             patch.object(settings, "SMTP_USERNAME", "test_user@example.com"), \
             patch.object(settings, "SMTP_PASSWORD", "test_pass"), \
             patch.object(sender, "_get_gmail_service", return_value=None):
            ...
            mock_smtp_class.assert_called_once_with("smtp.sendgrid.net", 587, timeout=20)
