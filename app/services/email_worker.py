...
class EmailOutreachWorker:
    ...
    async def set_sender_active(self, active: bool):
        self.is_sender_active = active
        if not active:
            self.status = "idle"
            self.current_lead = None
            await self.add_log("Auto-Sender process was manually paused by administrator.")
        else:
            self.wake_up()
            await self.add_log("Auto-Sender process was manually started by administrator.")
        await self.broadcast_activity()
    ...
    async def _process_autonomous_sending(self, db: Session):
        """
        Sends batches of personalized HTML emails to scraped leads autonomously.
        """
        if not self.is_sender_active:
        ...
        await self.broadcast_activity()
        await self.add_log(f"Auto-Sender: Starting dispatch for {len(leads)} leads...")

        # Process leads sequentially with immediate active state checking
        for lead in leads:
            if not self.is_sender_active:
                self.status = "idle"
                self.current_lead = None
                await self.broadcast_activity()
                await self.add_log("Auto-Sender: Dispatcher paused by user. Halting remaining queue.")
                return
            await self._send_single_lead(lead.id)
